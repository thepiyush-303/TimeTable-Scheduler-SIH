...
export class GeneticAlgorithm {
  ...
  generateInitialPopulation(): TimetableIndividual[] {
    const population: TimetableIndividual[] = [];
  ...
  private createRandomIndividual(): TimetableIndividual {
    ...
    const individual = new TimetableIndividual(entries);
    individual.calculateFitness(this.constraints);
    return individual;
  }

  // Generate all required timetable entries based on subjects and batches
  private generateRequiredEntries(): Array<{subject: Subject, batch: Batch, sessionsNeeded: number}> {
    const requirements: Array<{subject: Subject, batch: Batch, sessionsNeeded: number}> = [];
  ...
  private createRandomEntry(requirement: {subject: Subject, batch: Batch, sessionsNeeded: number}): TimetableEntry | null {
    ...
    if (suitableFaculties.length === 0) return null;
  ...
  evolve(): TimetableIndividual {
    ...
    while (generation < this.params.generations) {
      ...
      const newPopulation: TimetableIndividual[] = [];
      ...
      while (newPopulation.length < this.params.populationSize) {
        ...
        let offspring1: TimetableIndividual;
        let offspring2: TimetableIndividual;
  ...
  private tournamentSelection(population: TimetableIndividual[], tournamentSize: number = 3): TimetableIndividual {
    const tournament: TimetableIndividual[] = [];
  ...
  private crossover(parent1: TimetableIndividual, parent2: TimetableIndividual): [TimetableIndividual, TimetableIndividual] {
    ...
    return [
      new TimetableIndividual(offspring1Entries),
      new TimetableIndividual(offspring2Entries)
    ];
  }

  // Mutation operation
  private mutate(individual: TimetableIndividual): void {
    if (individual.entries.length === 0) return;
